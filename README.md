...
This repository contains an implementation of an OpenAI Universal MCP (Model Context Protocol) server. It provides a standardized interface for interacting with OpenAI's tools and services through a unified API.
...
The full list of available tools is at [./src/universal_mcp_openai/README.md](./src/universal_mcp_openai/README.md)
...
3. **Start the MCP Inspector**
   ...
   ```bash
   mcp dev src/universal_mcp_openai/server.py
   ```
...
4. **Install the Application**
   ```bash
   mcp install src/universal_mcp_openai/server.py
   ```
